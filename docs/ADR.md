...
**検討済み代替案（2026-07-20 追補）:** folders テーブルへの実体化（`folders(id, parent_id, name)` + 各テーブルから FK 参照）を再検討し、**棄却**した。仮想フォルダ概念とは背反せず（導出結果のマテリアライズとして両立可能）、リネーム・移動が1行 UPDATE になる利得はあるが、フォルダの「存在」が「配下サブツリーに kept な写真があるか」というサブツリー導出プロパティであるため、写真の trash / restore / purge / 移動のたびに祖先チェーンの存在状態を同期する義務が生まれ、同期漏れ＝ゴースト/消失フォルダというバグ源になる。現方式ではこの不変条件は定義により常に真（クエリが `Photo.kept` から導出）で、同期バグが構造的に存在しない。判断基準はワークロードで、現実（フォルダ約200・リネーム稀・削除/復元が日常・写真2.7万枚、path 重複のストレージコストは約1.3MB）では現方式が優位。**再検討のトリガー**: フォルダ移動が日常操作になる、または写真が現在の100倍規模になったとき。なお `folder_path` を持つ4テーブル（photos / folder_owners / access_rules / share_links）の値は重複データではなく、仮想フォルダの天然キー（文字列 PK）への参照である。整合性は FK ではなくアプリ層で担保（`FolderRenamer` の一括前置換とスナップショット突合テスト、孤児行の掃除）。
