...
**Decision:**
1. 画像は**認可付き自前エンドポイントから配信**する。素の `rails_*_path` は全廃し、URL 生成を `RendersPhotos#photo_image_path` に一本化（`ServesPhotoImage#stream_photo_image` が `send_blob_stream` でバイト列を流す。Thruster + local disk 前提）
