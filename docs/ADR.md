...
**検討済み代替案（2026-07-20 追補）:** folders テーブルへの実体化を2変種で検討し、いずれも**棄却**した。仮想フォルダ概念とは背反しない（導出結果のマテリアライズとして両立可能）。(a) **ライフサイクル状態を持つ変種**（folders に deleted_at 等）: フォルダの「存在」は「配下サブツリーに kept な写真があるか」の導出プロパティなので、写真の trash / restore / purge / 移動のたびに祖先チェーンの状態同期義務が生まれ、同期漏れ＝ゴースト/消失フォルダというバグ源になる。現方式ではこの不変条件は定義により常に真。(b) **文字列辞書としての interning 変種**（folders = id + path のみで状態なし、写真は FK + 自身の deleted_at、存在は join 越しの導出）: ゴミ箱ロジックは現方式と同型で成立する（同日再々検討で確認 — trash は (a) への反論であって (b) には当たらない）。ただしリネームは1行 UPDATE にならず（子孫フォルダ行のフルパス前置換が依然必要。2.7万行→約200行に縮むだけで同じロジック）、衝突チェックも両モデルで必要（マージを許す場合のみ差が出るが ADR-023 で拒否ポリシー）。新コストはアップロード毎の find-or-create、ACL/オーナーが指す中間パスにも行を要する規約、全クエリの join。**決定打はワークロード**: 現実（フォルダ約200・リネーム稀・削除/復元が日常・写真2.7万枚でリネーム全行書き換えは数十ms、path 重複のストレージコストは約1.3MB）では interning の利得が顕在化せず、移行コスト（4テーブル+ETL+spec 改修）に見合わない。グリーンフィールドで数百万枚規模なら (b) は妥当な選択。**再検討のトリガー**: フォルダ移動が日常操作になる、または写真が現在の100倍規模になったとき。なお `folder_path` を持つ4テーブル（photos / folder_owners / access_rules / share_links）の値は重複データではなく、仮想フォルダの天然キー（文字列 PK）への参照である。整合性は FK ではなくアプリ層で担保（`FolderRenamer` の一括前置換とスナップショット突合テスト、孤児行の掃除）。
...
## ADR-024: Ruby 型検査の限定導入（rbs-inline + Steep、コアロジックのみ）

**Status:** 承認済み ｜ **Date:** 2026-07-20

**Context:** 本プロジェクトには Ruby の型つき開発を試したいという狙いがある。一方で Rails はメタプログラミングと DSL が多く全面的な型付けはコストが高い。フロントは TypeScript 全面採用済み、backend は RSpec が安全網としてある。

**Decision:**
1. **対象は純粋ロジック3ファイルに限定**: `FolderPath` / `AccessPolicy` / `FolderRenamer`（Steepfile の `check` で明示）。コントローラ・AR モデル・ETL は対象外
2. **注釈は rbs-inline**（`#:` コメント形式）。実装と同じファイルに書き、`sig/generated/` に RBS を生成する。生成物もコミットする（差分レビューのため）
3. **検査は Steep**。ActiveRecord 境界は `sig/stubs/app_stubs.rbs` の最小手書きスタブで受け、クエリ面は `untyped`（Rails 全体の RBS 化はしない）
4. **実行は `bin/typecheck`**（生成→検査、DB 不要・数秒）。`bin/check` と CI（RuboCop の次段）に組み込み済み
5. RuboCop は `Layout/LeadingCommentSpace: AllowRBSInlineAnnotation: true` で `#:` を許可

**導入時の成果:** 初回の検査で `AccessPolicy.ancestor_chain` の `segments[0..i]`（Range 添字は nil を返し得る）が指摘され、nil を返さない `segments.take(i + 1)` へ書き換えた。実挙動のバグではないが、型検査が実際に届く証明にはなった。

**Consequences:** 対象3ファイルの変更時は注釈の保守が必要。対象を広げる場合はスタブの拡充（特に AR まわり）とのトレードオフを都度判断する。untyped 境界の内側（AR クエリの戻り値の扱い）は型検査の保証外で、そこは従来どおり RSpec が受け持つ。

## ADR-025: 本番配信 = Rails 単一コンテナ（nginx なし、SPA も Rails が配信）

**Status:** 承認済み ｜ **Date:** 2026-07-20

**Context:** 本番は手前に既設の nginx リバースプロキシがあり、TLS 終端・証明書更新はそちらの責務。旧構成（legacy）は puma + 静的配信 nginx の2コンテナだった。今回は SPA なのでフロント JS の配信をどこが担うかを決める必要がある。

**Decision:**
1. **アプリは Rails 単一コンテナ**（`compose.production.yaml` の `uprun_app`）。静的配信用 nginx は立てない
2. React ビルドはイメージビルド時に **`public/` へ焼き込み**（マルチステージ Dockerfile）、Rails が API・SPA・共有リンク OGP をすべて配信する。`/g/*` の OGP 注入（share_pages）が「Rails が index.html を配る」前提のため、フロント配信を nginx に分けるとむしろ壊れる
3. 静的配信の性能は **Thruster**（`bin/thrust`、:80 → Puma :3000）で担保。手前のリバプロは共有 external ネットワーク `web_network` 経由で `http://uprun_app:80` へプロキシ
4. SPA のクライアントルート直リンクは **catch-all（`SpaController`）**で index.html にフォールバック。ルート定義の最下部・HTML 要求のみ・`/api` 等は constraint でも除外して JSON 404 を維持
5. MySQL はコンテナ外（接続情報は `prod.envs`）。写真実体はボリューム `./data` に `STORAGE_ROOT=/data/storage` で配置。起動時に `db:prepare`（べき等）、ETL は手動（MIGRATION.md §3）

**Consequences:** コンテナ管理対象が1つで済み、旧構成の `volumes_from` による public 共有のような結合がない。Rails プロセスが静的配信も担うため、超高トラフィックになったら CDN か nginx 追加を再検討（現規模では不要）。

## ADR-026: 写真実体（バイト列）にもフォルダ可視性を強制する認可付き配信

**Status:** 承認済み ｜ **Date:** 2026-07-21

**Context:** JSON 一覧・メタデータは FolderQuery/AccessPolicy（ADR-005/006/019）で保護していたが、画像 URL は `rails_representation_path` / `rails_blob_path`（ActiveStorage 素の redirect 経路）を返していた。これは署名付きだが**認証も認可も行わない capability URL**で、`ActiveStorage::BaseController` 継承のためアプリの `require_approved` も通らない。結果として **restricted フォルダの写真も、URL さえ入手すれば非メンバー・ログイン外から実体を取得できる**穴になっていた（列挙は不可だが、リンク漏洩・横流し・失効回避が可能）。「JSON では隠すが実体は誰でも取れる」という非対称。

**Decision:**
1. 画像は**認可付き自前エンドポイントから配信**する。素の `rails_*_path` は全廃し、URL 生成を `RendersPhotos#photo_image_path` に一本化（`ServesPhotoImage#send_photo_image` が `send_file` で実体を返す。全環境 Disk サービス前提＝ADR-025、本番は Thruster の X-Sendfile 加速が効く）。**`ActiveStorage::Streaming`（`send_blob_stream`）は使わない** — `ActionController::Live` を混ぜ込み、dev のコードリロードとのデッドロックやスレッド枯渇で Puma 全体がハングし得る既知のリスクがあるため予防的に回避（Disk 配信では `send_file` が上位互換）
2. **認証経路** `GET /api/v1/photos/:id/image?variant=small|large|original`：`require_approved` の上で **JSON と同一の `FolderQuery#folder_visible?(photo.folder_path)`** を再利用（＝「JSON で見えるものだけ実体も見える」を定義上保証。restricted 非メンバーは弾かれる）。ゴミ箱（`deleted_at`）の実体は**所有者か admin のみ**。不可視・不存在は一律 404、未ログイン 401、未承認/Ban 403。キャッシュは `private`
3. **ゲスト経路** `GET /api/v1/g/:token/photos/:id/image`：トークン有効かつ写真が共有ルートの**サブツリー内**であること（ADR-008/009）。認証不要でトークン自体が capability。キャッシュは `public`
4. **OGP の `og:image` はゲスト経路を流用**（クローラは認証できない）。共有リンク＝全員閲覧の意味と一致し、素の capability URL よりむしろ厳密（トークン紐付け＋範囲制限）。カバー画像が公開されるのは共有作成時のオーナー意図の範囲

**Consequences:** 画像がアプリ経由になり CDN オフロードは失うが、local disk + Thruster 構成では実害なし。restricted/Ban/期限/trash が実体にも効くようになった。回帰テスト `spec/requests/api_photo_images_spec.rb`（非メンバー404・ゴミ箱・サブツリー外404 等）と OGP スペックが「JSON で見えない＝実体も取れない」を CI で常時固定する。フロントは URL 不透明利用のため改修不要。
